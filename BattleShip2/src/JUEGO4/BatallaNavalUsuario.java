...
public class BatallaNavalUsuario {
	int i,j,//counter
...
public BatallaNavalUsuario () throws IOException {
...
{		
	...
	{			
		...
		{
			BatallaNaval.getPlayers(1).setUser(in.readLine());	
		}				
		System.out.println("I'm still in the loop man.");
	}
	if (!start.equals("getmove"))			
	{
		BatallaNaval.getPlayers(0).setMove(true);
		JOptionPane.showMessageDialog(null,BatallaNaval.getPlayers(0).getUser()
		+" move.","",JOptionPane.INFORMATION_MESSAGE);
	}
	//BatallaNaval.getPlayers(0).getTimer().start();
}	
...
public void results()
{
	...
	{
		BatallaNaval.getPlayers(1).setBboard(x,y,Color.blue);
		BatallaNaval.getPlayers(0).setShots();
	}
	else if (results.startsWith("hit"))
	{
		BatallaNaval.getPlayers(0).setHits();			
		results=results.substring(results.indexOf(" ")+1);			
		if (results.startsWith("shipsunk"))
		{
			BatallaNaval.getPlayers(1).setShipsLeft();
			results=results.substring(results.indexOf(" ")+1);
			...
			{					
				for (i=x;i<=endp;i++)
				{
					BatallaNaval.getPlayers(1).setBboard(i,y,Color.black);
				}
			}
			else
			{
				for (i=y;i<=endp;i++)
				{
					BatallaNaval.getPlayers(1).setBboard(x,i,Color.black);
				}					
			}
		}			
		else
		{			
			BatallaNaval.getPlayers(1).setBboard(x,y,Color.orange);
		}
		BatallaNaval.getPlayers(0).setShots();			
	}
	else if (results.startsWith("theirshot"))
	{
		...
		x=Integer.parseInt(results.substring(0,1));
		y=Integer.parseInt(results.substring(2,3));
		if (!BatallaNaval.getPlayers(0).getWhatShip(x,y).equals(" "))
		{
			BatallaNaval.getPlayers(1).setHits();
			if (!BatallaNaval.getPlayers(0).isSunk(x,y
				,BatallaNaval.getPlayers(0).getWhatShip(x,y)))
				BatallaNaval.getPlayers(0).setBboard(x,y,Color.orange);
		}
		else
		  BatallaNaval.getPlayers(0).setBboard(x,y,Color.blue);
		BatallaNaval.getPlayers(1).setShots();				
	}
	...
	try
	{
	  ...
	  {
	  		BatallaNaval.setGameOver(true);
			if (BatallaNaval.getPlayers(0).getShipsLeft()!=0)
...
public void fireShot(int a, int b)
{
	...
	out.println(x);
	out.println(y);
	System.out.println("shot fired:  "+BatallaNaval.getCletters(x+1)+","+BatallaNaval.getCnumbers(y+1));		
}
...
{		
	out.println(BatallaNaval.getPlayers(BatallaNaval.getYou()).getUser());			
	for (i=0;i<10;i++)
		for (j=0;j<10;j++)
			out.println(BatallaNaval.getPlayers(BatallaNaval.getYou()).getWhatShip(i,j));		
}
}
