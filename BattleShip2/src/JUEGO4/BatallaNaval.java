...
private static int enemy=1,
	...
	dindex=0;//direction	
      ...
      private static Jugador players[]=new Jugador[2];
      ...
      private static Object selectedValue=" ",
			  gametype;
      private static BatallaNavalUsuario me;
      ...
      user=JOptionPane.showInputDialog("You have to input something.");
       ...
       if (dummy==3){
          JOptionPane.showMessageDialog(null,"Since you're having trouble inp"
           ...
           players[you]=new Jugador (user);
           players[enemy]=new Jugador ("Computer");						
           b=getContentPane();		
...
//variable that determines whether or not a patrol boat has been placed
      ...
      public static Jugador getPlayers(int x){
      return players[x];	
      }
...
//submenu of New Game
      ...
      m.add(pvp);
      pvc = new JMenuItem("Jugadorr vs. Computadora");
      pvc.addActionListener(stuff);
      m.add(pvc);
      cvc = new JMenuItem("Computadora vs. Computadora");
      ...
      JTextField k;		
         for (i=0;i<11;i++){			
	        for (j=0;j<11;j++){
		       ...
		       if (i==0){				
				...
				//used to display row of numbers
				  k= new JTextField(BatallaNaval.getCnumbers(j));
				  k.setEditable(false);
			...
			players[n].getMyBoard().add(k);
			   ...
			   k= new JTextField(BatallaNaval.getCletters(i));	
			   k.setEditable(false);
			   k.setHorizontalAlignment((int)JFrame.CENTER_ALIGNMENT); 
			   players[n].getMyBoard().add(k);
		       }				
...
//Listener for combo boxes used to layout ships 
	...
	if (players[you].getBoats(sindex) != null)
	{
		Barco boat=new Barco(ships[sindex],players[you].getBoats(sindex).getDirect()
...
//Listener for exit choice on Game menu	
	  ...
	  +"ike to exit BatallaNaval?", "Exit?", JOptionPane.YES_NO_OPTION);
	   if (r==0)
		System.exit(0);	
      }	
    }
...
//listener for New Game submenu		
	  ...
	  +"ike to start a new game?", "New Game?", JOptionPane.YES_NO_OPTION);
	   if (q==0){//q es para nuevo juego nuevo					
		//resets variables
		b.removeAll();
		c.removeAll();
		d.removeAll();				
		...
		if (gametype==pvp){
			...
			mbar.setText("BatallaNaval Demo");					
			mbar.setEditable(false);					
...
//Listener for ok button in statistics menu
      ...
      public static BatallaNavalUsuario getClient(){
      return me;		
 }

      public static void main(String[] args){		

      BatallaNaval gui= new BatallaNaval();
